 ...
 *
 */
public class ETaxbillSalesCooconServiceRunnable implements Runnable {
	...
	public ETaxbillSalesCooconServiceRunnable(JexData evdcInfo, String srch_gubn, String start_date, String end_date, String tax_agent_no, String tax_agent_password, String pay_yn) {
	        ...
	        else
	        {
	    		...
	    		try {
	        		...
	        		for(Object row : arr_resp_data)
	        		{
	        			...
	        			String rIssuId = resp_data.getString("APPROVAL_CODE");
	        			String ProviderRegNumber = (resp_data.getString("PROVIDER_REG_NUMBER")).replaceAll("[^0-9]","");
	        			String ProviderOtherRegno = resp_data.getString("PROVIDER_OTHER_REGNO");
	        			
	        			// 홈택스 전자세금계산서 조회 다량 조회 시 오류 발생
	        			if(("DETAILERROR".equals(ProviderRegNumber) && "80002F30".equals(ProviderOtherRegno)) 
	        					|| "80002E38".equals(ProviderOtherRegno)) {
	        			
	        				String issueDate = resp_data.getString("ISSUE_DATE");
	        				String sendDate = resp_data.getString("SEND_DATE");
	        				String ebillType = resp_data.getString("EBILL_TYPE");
	        				
	    					// 전자세금계산서 매입/매출 상세조회(실시간) 전문 호출(1336)
	        				JSONObject rsltDtlData = CooconApi.getEvdcTxblHstrDtlWithCertNameLongTerm(_biz_no, rIssuId, _cert_name, _tax_agent_no, _tax_agent_password);

	        				String rsltDtlCd = StringUtil.null2void(rsltDtlData.getString("ERRCODE"));
	        		    	String rsltDtlMsg = StringUtil.null2void(rsltDtlData.getString("ERRMSG"));

	        		        if("00000000".equals(rsltDtlCd)){
	        		        	resp_data = rsltDtlData;
	        		        	resp_data.put("ISSUE_DATE", issueDate);
	        		        	resp_data.put("SEND_DATE", sendDate);
	        		        	resp_data.put("EBILL_TYPE", ebillType);
	        		        	
	        		        	insertPtclData.add(insertElecTxblPtclData(resp_data, IssuId));
	        		        	
	        		        	JSONArray arr_det_data =  resp_data.getJSONArray("RESP_DATA");
	        		...
	        		if(!err){
		    			...
		    			// 매출세금계산서의 공급자정보로 아바타 사업장정보를 등록.
		    			if(arr_resp_data.size() > 0) {
		    				JSONObject hstr = (JSONObject) arr_resp_data.get(arr_resp_data.size()-1);
		    				insertInttData(hstr);
		    			}
	        			...
	        			idoCon.rollback();
	        			BizLogUtil.error(this, "1336 Error Code   ::"+ errCode);
	        			
	        			//오류인 경우 최종조회일자를 조회시작일자로 처리(재조회 가능하도록)
	        			his_lst_dtm = "";

	    				//최종조회일시 변경
	    				updEvdcSeupInfm(his_lst_dtm, errCode, errMsg);
	        		}
	        	} catch (Exception e) {
	...
	private void updEvdcSeupInfm(String last_dtm, String stts, String msg)
        ...
        JexData idoIn1 = util.createIDOData("EVDC_INFM_U012");
        //idoIn1.put("HIS_LST_DT"  , last_dt + SvcDateUtil.getShortTimeString());
	...
	private void insertInttData(JSONObject det) throws JexException, JexBIZException, Exception{
		...
		idoIn0.put("USE_INTT_ID", _use_intt_id);
		...
		if(StringUtil.isBlank(idoOut0.getString("BSNN_NM"))) {
	        ...
	        String latd  = "";	// 위도(y)
	       	String lotd  = "";	// 경도(x)
	       	String adrs = StringUtil.null2void(det.getString("PROVIDER_BUSINESS_PLACE"));
			...
			JexData idoIn1 = util.createIDOData("INTT_INFM_C001");
	        
	        idoIn1.put("USE_INTT_ID", _use_intt_id);
	        idoIn1.put("BIZ_NO", det.getString("PROVIDER_REG_NUMBER")); 							// 공급자 등록번호
	        idoIn1.put("BSNN_NM", StringUtil.toHalfChar(det.getString("PROVIDER_COMPANY_NAME"))); 	// 공급자 상호
	        idoIn1.put("RPPR_NM", StringUtil.toHalfChar(det.getString("PROVIDER_CEO_NAME"))); 		// 공급자 성명
	        idoIn1.put("ADRS", StringUtil.toHalfChar(det.getString("PROVIDER_BUSINESS_PLACE"))); 	// 공급자 사업장
	        idoIn1.put("LATD", latd); 																// 공급자 위도
	        idoIn1.put("LOTD", lotd); 																// 공급자 경도
	        idoIn1.put("BSST", StringUtil.toHalfChar(det.getString("PROVIDER_BUSINESS_CATEGORY"))); // 공급자 업태
	        idoIn1.put("TPBS", StringUtil.toHalfChar(det.getString("PROVIDER_BUSINESS_TYPE"))); 	// 공급자 종목
