...
import com.avatar.service.CorpCardCooconServiceRunnable;
...
/**
 * 법인카드 등록 후 실시간 조회. - Runnable 객체
 *
 * @author webcash.co.kr
 *
 */
public class CooconCardRealTime implements Runnable {

	private ExecutorService executorService;

	private JSONObject _cardInfo;
	private String _task_no;		// 작업일련번호(작업일시-yyyyMMddHHmmssSSS)
	
	public CooconCardRealTime(String task_no, JSONObject cardInfo) {
	...
	public void run() {
		...
		// 실시간조회등록작업 생성
     	insertRtInqTask(_cardInfo.getString("BANK_CD"), _cardInfo.getString("CARD_NO"));

		try {
			callRunnableCardService();
		} catch (Exception e) {
			// 쓰레드 비정상 종료
			BizLogUtil.error(this, _cardInfo.getString("USE_INTT_ID"), e, "Thread ["+Thread.currentThread().getName()+"]");
		} finally {
	 ...
	 * </pre>
	 */
	private void insertRtInqTask(String bank_cd, String card_no) {
		...
		try {
			...
			idoIn1.put("USE_INTT_ID"   , _cardInfo.getString("USE_INTT_ID"));
			idoIn1.put("TASK_NO"       , _task_no);
			idoIn1.put("TASK_GB"       , "2");		// 작업구분코드(1:계좌, 2:카드, 3:홈택스, 4:여신)
			idoIn1.put("TASK_STTS"     , "0");		// 작업상태(0:등록, 1:완료)
			idoIn1.put("BANK_CD"       , bank_cd);
			idoIn1.put("FNNC_INFM_NO"  , card_no);

			JexData idoOut1 =  idoCon.execute(idoIn1);
			if (DomainUtil.isError(idoOut1)) throw new JexBIZException(idoOut1);

		} catch (Exception e) {
			BizLogUtil.error(this, _cardInfo.getString("USE_INTT_ID")+".insertAcctInqTask()", e, "Thread ["+Thread.currentThread().getName()+"]");
		}
	}
	...
	private void updateRtInqTask() {
		...
		try {
			...
			BizLogUtil.error(this, _cardInfo.getString("USE_INTT_ID")+".updateAcctInqTask()", e, "Thread ["+Thread.currentThread().getName()+"]");
		}
	}
	 ...
	 */
	private void callRunnableCardService() throws JexException, JexBIZException {
			
		String start_date = SvcDateUtil.getInstance().getDate(-7, 'D');
        String end_date = SvcDateUtil.getInstance().getDate(-1, 'D');
     
        if("Y".equals(_cardInfo.getString("PAY_YN"))){
        	end_date = SvcDateUtil.getInstance().getDate();
        }
        
        // 법인카드 승인내역 실시간 조회 - Runnable 객체
    	Runnable task1 = new CorpCardCooconServiceRunnable(_cardInfo, start_date, end_date);
     	executorService.submit(task1);
     	
     	// 법인카드 한도금액 실시간 조회 - Runnable 객체
     	Runnable task2 = new CorpCardLimitCooconServiceRunnable(_cardInfo);
        ...
        // 신규등록/재등록 카드 목록조회 - 승인내역 실시간 조회용
		JexData idoIn1 = util.createIDOData("CARD_INFM_R018");
        idoIn1.put("USE_INTT_ID", _cardInfo.getString("USE_INTT_ID"));
        JexDataList<JexData> idoOutList = idoCon.executeList(idoIn1);

        if (DomainUtil.isError(idoOutList)) CommonErrorHandler.comHandler(idoOutList);

        //등록된 계좌수 만큼 Thread 등록
        for(JexData cardInfo : idoOutList) {
        	// 법인카드 승인내역 실시간 조회 - Runnable 객체
        	Runnable task = new CorpCardCooconServiceRunnable(cardInfo, start_date, end_date);
