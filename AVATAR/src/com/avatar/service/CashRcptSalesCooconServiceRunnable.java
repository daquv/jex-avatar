...
/**
 * 현금영수증 매출내역 실시간 조회 - Runnable 객체
 *
 * @author webcash.co.kr
 *
 */
public class CashRcptSalesCooconServiceRunnable implements Runnable {
	...
	public CashRcptSalesCooconServiceRunnable(JexData evdcInfo, String srchYYYYMM, String start_date, String end_date, String tax_agent_no, String tax_agent_password) {
	...
	public void run() {
		...
		try {
			//쿠콘 API 호출 - 현금영수증 매출내역조회 - 실시간, 등록된 인증서명
			JSONObject rsltData = CooconApi.getEvdcCashSelHstrWithCertName(_biz_no, _srchYYYYMM, _cert_name, _tax_agent_no, _tax_agent_password);
	        ...
	        else
	        {
	    		...
	    		try {
	        		...
	        		for(Object row : arr_resp_data)
	        		{
	        			...
	        			// 오늘일자 제외한 데이터만 저장
	        			if(!resp_data.getString("DATE_TRADE").substring(0, 8).equals(SvcDateUtil.getInstance().getDate())) {
	        				insertData.add(insertRcptSelPtclData(resp_data));
	        			}
	        		}
	...
	private void deleteRcptSelPtclData(JexConnection idoCon)
			throws JexException, JexBIZException {

		JexCommonUtil util = JexContext.getContext().getCommonUtil();

		//현금영수증 매출내역조회 - 실시간(1334)는 무조건 3개월치 조회하기 때문에, 그  기간만큼 삭제해줘야 함.
	...
	private JexData insertRcptSelPtclData(JSONObject hstrData) throws JexException, JexBIZException
	{
		JexCommonUtil util = JexContext.getContext().getCommonUtil();
		...
		idoIn1.put("USE_INTT_ID", _use_intt_id);
		idoIn1.put("TRNS_DT"    , hstrData.getString("DATE_TRADE").substring(0, 8));
		idoIn1.put("APV_NO"     , hstrData.getString("APPROVAL_CODE")); // 승인번호
		idoIn1.put("BILL_DTM"  , hstrData.getString("DATE_TRADE"));
		...
		idoIn1.put("ISSU_GB"	, hstrData.getString("GUBUN_NM"));
		idoIn1.put("MEMO"       , "");
	...
	private void updEvdcSeupInfm(String last_dtm, String stts, String msg)
        ...
        //idoIn1.put("HIS_LST_DTM"  , last_dt + SvcDateUtil.getShortTimeString());
