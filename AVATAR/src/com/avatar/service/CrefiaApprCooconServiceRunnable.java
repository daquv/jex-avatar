 ...
 *
 */
public class CrefiaApprCooconServiceRunnable implements Runnable {
	...
	public CrefiaApprCooconServiceRunnable(JexData evdcInfo, String start_date, String end_date) {
	...
	public void run() {
		...
		try {
			//쿠콘 API 호출 - 여신금융협회 가맹점 매출내역 통합조회 - 매출승인내역, 실시간
			JSONObject rsltData = CooconApi.getCardSalesApprHstr(_web_id, _web_pwd, _start_date, _end_date, _branch_name);
	    		...
	    		try {
	        		...
	        		for(Object row : arr_resp_data)
	        		{
	        			...
	        			for(int j=0 ; j < arr_dtl_resp_data.size() ; j++)
        				{
        					...
        					insertData.add(insertApvHisData(dtl_resp_data));
        				}
	        		}
	 ...
	 */
	private JexData insertApvHisData(JSONObject hstrData) throws JexException, JexBIZException
	{
		JexCommonUtil util = JexContext.getContext().getCommonUtil();

		JexData idoIn1 = util.createIDOData("CARD_SEL_APV_HSTR_C001");

		idoIn1.put("USE_INTT_ID"  , _use_intt_id);
		idoIn1.put("TRNS_DT"      , hstrData.getString("DATE_TRADE_DETAIL").trim());	// 거래일자
		idoIn1.put("CARD_NO"      , hstrData.getString("CARD_NUM").trim());				// 카드번호
		idoIn1.put("APV_NO"       , hstrData.getString("APPROVAL_NUM").trim());			// 승인번호
		idoIn1.put("DIV"          , hstrData.getString("AUTHGBN").trim());				// 거래구분
		idoIn1.put("TRNS_TIME"    , hstrData.getString("TIME_TRADE").trim());			// 거래시간
		idoIn1.put("APV_AMT"      , hstrData.getString("APPROVAL_AMOUNT").trim());		// 승인금액
		idoIn1.put("ITLM_TERM_CNT", hstrData.getString("INSTALLMENT_DIVID").trim());	// 할부기간
	...
	private void updEvdcSeupInfm(String last_dtm, String stts, String msg)
        ...
        JexData idoIn1 = util.createIDOData("EVDC_INFM_U013");
        //idoIn1.put("APV_HIS_LST_DTM"  , last_dt + SvcDateUtil.getShortTimeString());
        idoIn1.put("RT_APV_HIS_LST_DTM" , last_dtm);
        idoIn1.put("APV_HIS_LST_STTS", stts);
        idoIn1.put("APV_HIS_LST_MSG" , msg);
        idoIn1.put("USE_INTT_ID"     , _use_intt_id);
	...
	private void deleteApvHisData(JexConnection idoCon)
		...
		JexData idoIn1 = util.createIDOData("CARD_SEL_APV_HSTR_D001");
