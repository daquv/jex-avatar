...
public class AccountLoanAmtRealTimeCollectorSub extends BatchServiceBiz {

	private static final String job_id = "GET_RT_ACCT_LOAN_AMT";
	...
	private void acctAmtReg_intt(JexConnection idoCon, JexCommonUtil util, String use_intt_id, String bsnn_no)
		...
		JSONObject resData = CooconApiMgnt.data_wapi_0150("", "", bsnn_no);
		...
		for (Object resp : resp_data) {
			...
			for (int j = hstr_data.size() - 1; j >= 0; j--) {
				...
				int_payt_dt = hstrData.getString("DATE_PAY_INTEREST");	// 이자납입일
				expi_dt = hstrData.getString("DATE_EXPIRE");			// 만기일자			
				
				// 잔액 변경
				updateAcctAmt(idoCon, util, use_intt_id, fnnc_unq_no, bal, 
						cont_rt, agmt_amt, pyat_amt, pyat_dt, int_payt_dt, expi_dt, rsltCd, rsltMsg);
	...
	private void updateAcctAmt(JexConnection idoCon, JexCommonUtil util, String use_intt_id, String fnnc_unq_no,
			String bal, String cont_rt, String agmt_amt, String pyat_amt, String pyat_dt,
			String int_payt_dt, String expi_dt, String rslt_cd, String rslt_msg) throws JexBIZException, JexException {
		
		JexData idoIn1 = util.createIDOData("ACCT_INFM_U019");
		...
		idoIn1.put("INT_PAYT_DT"  , int_payt_dt);
	...
	private void updateLstStts(JexConnection idoCon, JexCommonUtil util, String use_intt_id, String fnnc_unq_no,
		...
		dynamic1.addSQL("AND COALESCE(ACCT_DV , '01') = '03'");
	...
	private int getAcctCount(JexConnection idoCon, JexCommonUtil util, String use_intt_id)
		...
		JexData idoIn1 = util.createIDOData("ACCT_INFM_R020");
		idoIn1.put("USE_INTT_ID", use_intt_id);
		JexData idoOut1 = idoCon.execute(idoIn1);
