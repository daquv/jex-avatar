...
/**
 * 매시간 쇼핑몰(배달앱) 주문내역  조회 서브
 *
 * @author won
 *
 */
public class SnssOrdrRealTimeCollectorSub extends BatchServiceBiz {

	private static final String job_id = "GET_RT_SNSS_ORDR";
	...
	private void insSnssOrdrHstr(JexData shop_info, String use_intt_id, String bsnn_no)
		...
		// 거래상태 변경 여부 반영을 위해 매시간 배치도 조회할때마다 3일치씩 조회(ex.완료 -> 취소)
		if (!"".equals(startDate)) {
		...
		if (!"00000000".equals(resData.getString("ERRCODE")) && !"42110000".equals(resData.getString("ERRCODE"))) {
			// 상태 업데이트
			updEvdcSeupInfm(use_intt_id, shopCd, subShopCd, resData.getString("ERRCODE"), resData.getString("ERRMSG"));
			// 매시간 쇼핑몰 상태값 변경
			updateCustRtSnssOrdrHisLst(use_intt_id, resData.getString("ERRCODE"), resData.getString("ERRMSG"));
		...
		// 최종 상태 업데이트
		updEvdcSeupInfm(use_intt_id, shopCd, subShopCd, rsltCd, rsltMsg);
		// 매시간 쇼핑몰 상태값 변경
		updateCustRtSnssOrdrHisLst(use_intt_id, rsltCd, rsltMsg);
	 ...
	 * @throws JexBIZException
	 */
	private void updEvdcSeupInfm(String use_intt_id, String shop_cd, String sub_shop_cd, String stts, String msg)
			throws JexException, JexBIZException {
	 ...
	 * @throws JexException
	 */
	private void updateCustRtSnssOrdrHisLst(String use_intt_id,
