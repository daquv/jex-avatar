...
public class AccountAmtCollectorSub extends BatchServiceBiz {

	private static final String job_id = "GET_ACCT_AMT";
	...
	public JexData execute(JexData input) throws JexException, JexBIZException, Throwable {
		...
		// 계좌별 잔액조회
		//acctAmtReg_acct(idoCon, util, use_intt_id, bsnn_no);
	 ...
	 * @throws JexException
	 */
	private void acctAmtReg_acct(JexConnection idoCon, JexCommonUtil util, String use_intt_id, String bsnn_no)
			throws JexBIZException, JexException {
		...
		for (JexData acctInfo : idoOut1) {
			...
			JSONObject resData = CooconApiMgnt.data_wapi_0108(acctInfo.getString("BANK_CD"), acctInfo.getString("FNNC_INFM_NO"), bsnn_no);

			if (!"00000000".equals(resData.getString("ERRCODE"))) {
				updateLstStts(idoCon, util, use_intt_id, acctInfo.getString("FNNC_UNQ_NO"), resData.getString("ERRCODE"), resData.getString("ERRMSG"));
				batchvo.addErrCnt(1);
				batchvo.setProc_stts(resData.getString("ERRCODE"));
				batchvo.setProc_rslt_ctt(resData.getString("ERRMSG"));				
				continue;
			...
			for (Object resp : resp_data) {
				...
				if ("WERR0006".equals(rsltCd)) {
					batchvo.addEtcCnt(1);
					rsltCd = "0000";
					updateLstStts(idoCon, util, use_intt_id, acctInfo.getString("FNNC_UNQ_NO"), rsltCd, rsltMsg);
					break;
				}
				...
				JSONObject hstrData = null;
				JSONArray hstr_data = respData.getJSONArray("HSTR_DATA");
				
				for (int j = 0; j < hstr_data.size(); j++) {
					hstrData = hstr_data.getJSONObject(j);

					real_amt = hstrData.getString("AMOUNT_DRAWABLE");	// 출금가능액
					bal = hstrData.getString("AMOUNT_BALANCE");			// 계좌잔액

					// 잔액 변경
					updateAcctAmt(idoCon, util, use_intt_id, acctInfo.getString("FNNC_UNQ_NO"), bal, real_amt, rsltCd, rsltMsg);
				}
				batchvo.addNorCnt(1);
			}
		}
	...
	private void updateAcctAmt(JexConnection idoCon, JexCommonUtil util, String use_intt_id, String fnnc_unq_no,
		...
		JexData idoIn1 = util.createIDOData("ACCT_INFM_U011");
		idoIn1.put("BAL"		  , bal);
