...
/**
 * 매시간 현금영수증 매출 내역 조회 서브
 * 
 * @author won
 *
 */
public class CashRcptSalesRealTimeCollectorSub extends BatchServiceBiz {

	private static final String job_id = "GET_RT_CASH_RCPT_SALES";
	...
	public JexData execute(JexData input) throws JexException, JexBIZException, Throwable {
		...
		if (DomainUtil.getResultCount(evdc_data) == 1) {
			...
			if (!"00000000".equals(resData.getString("ERRCODE"))) {
				...
				// 매시간 현금영수증 매출조회 상태값 변경
				updateCustRtRcptSalesHisLst(idoCon, use_intt_id, resData.getString("ERRCODE"), resData.getString("ERRMSG"), SvcDateUtil.getInstance().getDate(startDate, -1, 'D'));
			...
			// 매시간 현금영수증 매출조회 상태값 변경
			updateCustRtRcptSalesHisLst(idoCon, use_intt_id, rsltCd, rsltMsg, rsltDt);
	 ...
	 * @throws JexException
	 */
	private void updateCustRtRcptSalesHisLst(JexConnection idoCon, String use_intt_id,
