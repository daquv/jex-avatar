 ...
 *
 */
public class CashRcptPerchaseCollectorSub extends BatchServiceBiz {
	...
	public JexData execute(JexData input) throws JexException, JexBIZException, Throwable {
		...
		if (batch_gb.equals("W")) {
			job_id = "GET_CASH_RCPT_PRCHS_W";
		} else {
			job_id = "GET_CASH_RCPT_PRCHS_M";
		}
		...
		String endDate = previous_month + Integer.toString(SvcDateUtil.getInstance().getLastDay(previous_month)); // 전월
		...
		if (DomainUtil.getResultCount(evdc_data) == 1) {
			...
			if (!"00000000".equals(resData.getString("ERRCODE"))) {
				// 최종조회일시, 상태 업데이트
				if ("W".equals(batch_gb)) {
					if("".equals(StringUtil.null2void(evdc_data.getString("BUY_HIS_LST_DTM")))) {
			...
			if (resData.getJSONArray("RESP_DATA") != null) {
				...
				for (Object resp : resData.getJSONArray("RESP_DATA")) {
					...
					for (int i = 0; i < hstr_data.size(); i++) {
						...
						// 주 배치
						if ("W".equals(batch_gb)) {
							insertData.add(insertRcptBuyPtclData(hstrData, use_intt_id));
						}
						// 월 배치
						else {
	 ...
	 * @throws JexBIZException
	 */
	private JexData insertRcptBuyPtclData(JSONObject hstrData, String use_intt_id)
			throws JexException, JexBIZException {
		JexCommonUtil util = JexContext.getContext().getCommonUtil();
