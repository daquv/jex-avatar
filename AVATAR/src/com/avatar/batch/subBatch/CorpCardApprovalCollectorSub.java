 ...
 *
 */
public class CorpCardApprovalCollectorSub extends BatchServiceBiz {

	private final static String job_id = "GET_CORP_CARD_APRV";
	...
	public JexData execute(JexData input) throws JexException, JexBIZException, Throwable {
		...
		while (cardList.next()) {
			...
			// 호출 이력
//			extnInsert(use_intt_id, "C", "0106", errcode, errmsg);
	 ...
	 * @param card_no
	 * @param card_stts
	 * @param apv_his_lst_dt
	 * @param apv_his_lst_stts
	 * @param apv_his_lst_msg
	 * @throws JexException
	 * @throws JexBIZException
	 */
	private void updateCardApvHisLst(String use_intt_id, String card_no, String apv_his_lst_dt,
			String apv_his_lst_stts, String apv_his_lst_msg) throws JexException, JexBIZException {
		JexConnection idoCon = JexConnectionManager.createIDOConnection();
		JexCommonUtil util = JexContext.getContext().getCommonUtil();

		if (apv_his_lst_dt.length() == 8) {
			apv_his_lst_dt = apv_his_lst_dt + SvcDateUtil.getShortTimeString();
		}

		JexData idoIn1 = util.createIDOData("CARD_INFM_U002");
		//idoIn1.put("PTL_ID"          , CommUtil.getPtlId());
		idoIn1.put("USE_INTT_ID"     , use_intt_id);
		idoIn1.put("CARD_NO"         , card_no);
		idoIn1.put("CARD_STTS"       , "1");
		idoIn1.put("APV_HIS_LST_DTM"  , apv_his_lst_dt);
