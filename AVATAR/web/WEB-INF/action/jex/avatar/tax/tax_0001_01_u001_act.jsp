...
<%
         ...
         *
         * @File Title   : 홈택스 증빙설정 수정
         * @File Name    : tax_0001_01_u001_act.jsp
         * @File path    : tax
         * @author       : kth91 (  )
         * @Description  : 홈택스 증빙설정 수정
         * @Register Date: 20200203101444
         * </pre>
         *
         * ============================================================
         * 참조
        ...
        @JexDataInfo(id="tax_0001_01_u001", type=JexDataType.WSVC)
    	...
    	String other_stts = StringUtil.null2void(input.getString("OTHER_STTS"));
...
//     	JSONObject resData0 = new JSONObject();
    	...
    	// 해당사업자에 대한 인증서만 정보 가져올수 있음.
//     	resData0 = CooconApi.gateway_1506(bsnn_no_t, cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data);
         
    	// 인증서 등록일 경우 검증 방법
    	if(reg_type.equals("1")){
    		// 인증서검증(1335:전자세금계산서 매입/매출 목록조회)
    		resData1 = CooconApi.getEvdcTxblHstrWithCertInfo("3", strToday, strToday, cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, "", "");
    		// 외부이력테이블에 응답 결과 이력 입력
//             ExtnTrnsHis.insert(use_intt_id, "C", "1335", resData1.getString("RESULT_CD"), resData1.getString("RESULT_MG"));
    		
    		strResData1 = (String)resData1.get("RESULT_CD");
    	}else{
    		// 등록된 인증서일 경우 검증 방법(인증서명 이용)
    		// 인증서검증(1335:전자세금계산서 매입/매출 목록조회)
    		resData1 = CooconApi.getEvdcTxblHstrWithCertName(bsnn_no, "3", strToday, strToday, cert_name, "", "");
    		// 외부이력테이블에 응답 결과 이력 입력
//             ExtnTrnsHis.insert(use_intt_id, "C", "1335", resData1.getString("ERRCODE"), resData1.getString("ERRMSG"));
    		
    		strResData1 = (String)resData1.get("ERRCODE");
    	}
    	
    	if(!"00000000".equals(strResData1) && 
    		!"4".equals(StringUtil.null2void(strResData1).substring(0, 1))){
    		bTaxPayerRegNo = true;
    		// 인증서 등록일 경우 검증 방법
    		if(reg_type.equals("1")){
    			// 인증서검증(1335:전자세금계산서 매입/매출 목록조회)
    			resData1 = CooconApi.getEvdcTxblHstrWithCertInfo("3", strToday, strToday, cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, "4|"+bsnn_no, "", "");
    			// 외부이력테이블에 응답 결과 이력 입력
//                 ExtnTrnsHis.insert(use_intt_id, "C", "1335", resData1.getString("RESULT_CD"), resData1.getString("RESULT_MG"));
    		...
    		// 등록된 인증서일 경우 검증 방법(인증서명 이용)
    			...
    			// 인증서검증(1335:전자세금계산서 매입/매출 목록조회)
    			resData1 = CooconApi.getEvdcTxblHstrWithCertName(bsnn_no, "3", strToday, strToday, cert_name, "4|"+bsnn_no, "", "");
    			// 외부이력테이블에 응답 결과 이력 입력
//                 ExtnTrnsHis.insert(use_intt_id, "C", "1335", resData1.getString("ERRCODE"), resData1.getString("ERRMSG"));
    	...
    	// 홈택스 증빙설정정보 상태 조회
    	JexData idoIn6 = util.createIDOData("EVDC_INFM_R009");
    
        idoIn6.put("USE_INTT_ID", use_intt_id);
        idoIn6.put("EVDC_DIV_CD1", input.getString("EVDC_DIV_CD1"));
        idoIn6.put("EVDC_DIV_CD2", input.getString("EVDC_DIV_CD2"));
        ...
        stts = StringUtil.null2void(idoOut6.getString("STTS"));				// 전자(세금)계산서 상태값
    	other_stts = StringUtil.null2void(idoOut6.getString("OTHER_STTS")); // 현금영수증 상태값
        
    	// 증빙설정정보 저장되어 있는 값이 없으면 최초등록
    	if("".equals(stts) && "".equals(other_stts)){
    	...
    	if("".equals(stts)){
	        ...
	        idoIn7.put("EVDC_DIV_CD", input.getString("EVDC_DIV_CD1"));
	        ...
	        idoIn8.put("EVDC_DIV_CD", input.getString("EVDC_DIV_CD1"));
    	...
    	String taxpayer_regno = "";
		if(bTaxPayerRegNo){
			taxpayer_regno = "4|"+bsnn_no; // 납세자 사업자번호
		}
    	
    	if("1".equals(stts)){
    		//수정
    		resData2 = CooconApi.updateEvdcTxbl(bsnn_no, bsnn_no, cert_name, before_cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, cert_reg_type, "1", taxpayer_regno, "", "");
    		// 외부이력테이블에 응답 결과 이력 입력
//             ExtnTrnsHis.insert(use_intt_id, "C", "0100_006_U", resData2.getString("ERRCODE"), resData2.getString("ERRMSG"));
    	}else{
    		//등록
    		resData2 = CooconApi.insertEvdcTxbl(bsnn_no, bsnn_no, cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, cert_reg_type, "1", taxpayer_regno, "", "");
    		// 외부이력테이블에 응답 결과 이력 입력
//     		ExtnTrnsHis.insert(use_intt_id, "C", "0100_006_I", resData2.getString("ERRCODE"), resData2.getString("ERRMSG"));
    	}
    	...
    	if("00000000".equals(resData2.get("ERRCODE"))||"WSND0004".equals(resData2.get("ERRCODE"))){// 성공
        	...
        	cert_pwd = CommUtil.getDecrypt(decryptKey, StringUtil.null2void(input.getString("CERT_PWD")));
        	before_cert_name = StringUtil.null2void(input.getString("BEFORE_CERT_NAME"));
        	cert_name = StringUtil.null2void(input.getString("CERT_NAME"));
        	cert_org = StringUtil.null2void(input.getString("CERT_ORG"));
        	cert_date = StringUtil.null2void(input.getString("CERT_DATE"));
        	cert_folder = StringUtil.null2void(input.getString("CERT_FOLDER"));
        	cert_data = StringUtil.null2void(input.getString("CERT_DATA"));
        	
    		if("".equals(other_stts)){
        		// 현금영수증  증빙설정정보 등록
        		JexData idoIn9 = util.createIDOData("EVDC_INFM_C002");
        
        		idoIn9.put("USE_INTT_ID", use_intt_id);
        		idoIn9.put("EVDC_DIV_CD", input.getString("EVDC_DIV_CD2"));
        		idoIn9.put("BIZ_NO", bsnn_no);
        		idoIn9.put("CERT_NM", cert_name);
        		idoIn9.put("CERT_ORG", cert_org);
        		idoIn9.put("CERT_DT", cert_date);
        		idoIn9.put("STTS", "1");
        		idoIn9.put("REGR_ID", user_id);
        		idoIn9.put("CORR_ID", user_id);
    			...
    			// 현금영수증  증빙설정정보 변경
        		JexData idoIn10 = util.createIDOData("EVDC_INFM_U008");
        
        		idoIn10.put("CERT_NM", cert_name);
        		idoIn10.put("CERT_ORG", cert_org);
        		idoIn10.put("CERT_DT", cert_date);
        		idoIn10.put("STTS", "1");
        		idoIn10.put("CORR_ID", user_id);
        		idoIn10.put("USE_INTT_ID", use_intt_id);
        		idoIn10.put("EVDC_DIV_CD", input.getString("EVDC_DIV_CD2"));
        		idoIn10.put("BIZ_NO", bsnn_no);
    		...
    		// 변경전 인증서명(거래구분코드가 "U:수정"인 경우만 입력)
    		if(!"1".equals(other_stts)){
    			before_cert_name = "";
    		}

    		if("1".equals(other_stts)){
    			resData2 = CooconApi.updateEvdcCash(bsnn_no, bsnn_no, cert_name, before_cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, cert_reg_type, "1", "", "");
    			// 외부이력테이블에 응답 결과 이력 입력
//     			ExtnTrnsHis.insert(use_intt_id, "C", "0100_002_U", resData2.getString("ERRCODE"), resData2.getString("ERRMSG"));
    		}else{
    			resData2 = CooconApi.insertEvdcCash(bsnn_no, bsnn_no, cert_name, cert_org, cert_date, cert_pwd, cert_folder, cert_data, cert_reg_type, "1", "", "");
    			// 외부이력테이블에 응답 결과 이력 입력
//     			ExtnTrnsHis.insert(use_intt_id, "C", "0100_002_I", resData2.getString("ERRCODE"), resData2.getString("ERRMSG"));
    		}
    		...
    		if("00000000".equals(resData2.get("ERRCODE"))||"WSND0004".equals(resData2.get("ERRCODE"))){// 성공   
                ...
                // 아바타 유료회원 신청/해지(현금영수증 : 002, 전자세금계산서 : 006)
                JSONObject resJData = CooconApi.insertMembership("002", use_intt_id, pay_yn);
                
              	//(00000000:정상, WSND0004:이미 등록된 내용이 있습니다.)
                if("00000000".equals(resJData.get("ERRCODE")) || "WSND0004".equals(resJData.get("ERRCODE"))){
                	BizLogUtil.debug(this, "아바타 현금영수증 유료회원 신청/해지 정상처리되었습니다. (변경데이터 : "+pay_yn+")");   
                	// 006:전자세금계산서 으로 한번 호출해야됨.
                	...
                	if("00000000".equals(resJData2.get("ERRCODE")) || "WSND0004".equals(resJData2.get("ERRCODE"))){
                		BizLogUtil.debug(this, "아바타 전자세금계산서 유료회원 신청/해지 정상처리되었습니다. (변경데이터 : "+pay_yn+")");
                	}else{
                		BizLogUtil.debug(this, "아바타 전자세금계산서 유료회원 신청/해지 중 오류가 발생하였습니다. (변경데이터 : "+pay_yn+")");
                	}
                }else{
                	BizLogUtil.debug(this, "아바타 현금영수증 유료회원 신청/해지 중 오류가 발생하였습니다. (변경데이터 : "+pay_yn+")");
                }
        		...
        		String tmplId = "askavatar_003_hometax_renew";
