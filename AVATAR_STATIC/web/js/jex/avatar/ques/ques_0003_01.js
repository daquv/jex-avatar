 ...
 *
 * @File Name      : ques_0003_01.js
 * @File path      : AVATAR_STATIC/web/js/jex/avatar/ques
 * @author         : 김별 (  )
 * @Description    : 입/출금내역화면/조회
 * @History        : 20200122171755, 김별
 * </pre>
 **/
var date = "";
var date1 = "";
...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"] = date;
			input["END_DT"] = date1;
			avatar.common.callJexAjax("ques_0003_01_r001", input, _thisPage.fn_callback, "", "c");
		},
		...
		fn_callback : function(data){
			...
			if(data.REC2.length == 0){
				...
				if(data.REC.length == 0){
					$("li:eq(0) .right").html('0원');
					$(".aias_news .c_blue").html('0원');
					
					$("li:eq(1) .right").html('0원');
					$(".aias_news .c_red").html('0원');
					...
					$.each(data.REC, function(idx, rec){
						if(rec.INOT_DSNC == '1'){
							inHtml = '';
							inHtml +='<div class="card">';
							inHtml +='	<h2>'+avatar.common.null2void(rec.BANK_NM)+'</h2>';
							inHtml +='	<div class="right">';
							inHtml += 		avatar.common.comma(rec.TOTL_AMT, true)+' 원';
							inHtml +='	</div>';
							inHtml +='</div>';
							
							intotlamt += parseInt(rec.TOTL_AMT);
							$("li:eq(0) .sub").append(inHtml);
						}
						else if(rec.INOT_DSNC == '2'){
							outHtml = '';
							outHtml +='<div class="card">';
							outHtml +='	<h2>'+avatar.common.null2void(rec.BANK_NM)+'</h2>';
							outHtml +='	<div class="right">';
							outHtml += 		avatar.common.comma(rec.TOTL_AMT, true)+' 원';
							outHtml +='	</div>';
							outHtml +='</div>';
							
							outtotlamt += parseInt(rec.TOTL_AMT);
							$("li:eq(1) .sub").append(outHtml);
							
						}
					});
					
					$("li:eq(0) .right:eq(0)").html(avatar.common.comma(intotlamt, true)+' 원');
					$(".aias_news .c_blue").html(avatar.common.comma(intotlamt, true)+' 원');
					
					$("li:eq(1) .right:eq(0)").html(avatar.common.comma(outtotlamt, true)+' 원');
					$(".aias_news .c_red").html(avatar.common.comma(outtotlamt, true)+' 원');
					if(avatar.common.null2void(date1)!="")
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1));
					else{
						$(".aias_news strong").html(date_format_kr(date));
