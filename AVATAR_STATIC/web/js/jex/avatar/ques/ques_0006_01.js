 ...
 *
 * @File Name      : ques_0006_01.js
 * @File path      : AVATAR_STATIC/web/js/jex/avatar/ques
 * @author         : 김별 (  )
 * @Description    : 카드매출입금예정금액화면/조회
 * @History        : 20200122131226, 김별
 * </pre>
 **/
var date = "";
var date1 = "";
...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"] = date;
			input["END_DT"] = date1;
			avatar.common.callJexAjax("ques_0006_01_r001", input, _thisPage.fn_callback, "false", "c");
		},
		...
		fn_callback : function(data){
			...
			if(data.REC2[1].CNT==0){
				...
				if(data.REC.length == 0){
					...
					$(".aias_news td .c_blue").html("0원");
				} else{
					var totlAmt = 0;
					$.each(data.REC, function(idx, rec){
						var html = '';
						html += '<dd>';
						html += 	rec.CARD_CORP_NM;
						html += '	<div class="right">';
						html += 		avatar.common.comma(rec.SEL_SUM, true)+"원";
						html += '	</div>';
						html += '</dd>';
						
						$(".aias_list dl").append(html);
						
						totlAmt += parseInt(rec.SEL_SUM);
					});
					$(".aias_news td .c_blue").html(avatar.common.comma(totlAmt, true)+"원");
					if(avatar.common.null2void(date1)!=""){
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1));
					}else{
						$(".aias_news strong").html(date_format_kr(date));
		...
		fn_calDate : function(){
			if($("#NE_DAY").val())	{
				day = $("#NE_DAY").val();
				if(day == "당일" || day =="전일"){
					date=avatar.common.getDate("yyyymmdd");
				}
				else if(day == "전주"){
				...
				date = String(avatar.common.getDate("yyyymmdd"));
			}
		},
