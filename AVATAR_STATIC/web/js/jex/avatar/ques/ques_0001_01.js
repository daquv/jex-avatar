...
$(function(){
	_thisPage.onload();
	/*iWebAction("setStorage",{"_key" : "dateDayModal1", "_value" : ''});
	iWebAction("setStorage",{"_key" : "dateDayModal2", "_value" : ''});*/
	$(document).on("click", "[name=quesList]  ul li", function(){
		var nativeIntent = ",NNN002,NNN004,NNN012,NNN015," 
		if(nativeIntent.indexOf($(this).attr('intent'))>-1){
			...
			//var url = "basic_0009_02.act?MENU_DV=NATIVE";
			//iWebAction("openPopup",{"_url" : url});
			// 전화해줘 Native화면으로 이동
	...
	//메모해줘 화면으로 이동처리
	$(document).on("click", "#memoReg_move", function(){
		var url = "ques_0014_02.act";
		iWebAction("openPopup",{"_url" : url});
	});
	
	//메모보여줘 화면으로 이동처리
	$(document).on("click", "#memoList_move", function(){
		var url = "ques_0014_01.act";
		iWebAction("openPopup",{"_url" : url});
	});
	
	//데이터 연동하기 
	$(document).on("click", "#dataConnect", function(){
		var url = "basic_0002_01.act";
		iWebAction("openPopup",{"_url" : url});
	});
	
	//질문 모아보기 더보기
	$(document).on("click", "#myRecentQues .bt_more", function(){
		var url = "basic_0013_01.act?MENU_DV=01";
		iWebAction("openPopup",{"_url" : url});
	});
	//답변받지 못한 질문 더보기
	$(document).on("click", "#myUnansweredQues .bt_more", function(){
