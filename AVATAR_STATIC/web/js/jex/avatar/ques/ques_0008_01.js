 ...
 *
 * @File Name      : ques_0008_01.js
 * @File path      : AVATAR_STATIC/web/js/jex/avatar/ques
 * @author         : 김별 (  )
 * @Description    : 최고매출거래처조회화면
 * @History        : 20200203111430, 김별
 * </pre>
 **/
var date = "";
var date1 = "";
...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"]		= date;
			input["END_DT"]		= date1;
			avatar.common.callJexAjax("ques_0008_01_r001",input,_thisPage.fn_callback, "", "c");
		},
		...
		fn_callback : function(data){
			if(data.REC2[0].CNT == 0){
				...
				if(data.REC.length == 0){
					$(".card:eq(0) .right").html("0원");
					$(".sub").hide();
					$(".aias_news td span:eq(0)").html("");
					$(".aias_news td span:eq(1)").html("0원");
					if(avatar.common.null2void(date1)!="")
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1) +" 최고매출 거래처는");
					else{
						$(".aias_news strong").html(date_format_kr(date) +" 최고매출 거래처는");
					}
				} else {
					var totlAmt;
					$.each(data.REC, function(idx, rec){
						$(".card:eq(0) .right").html(avatar.common.comma(rec.MAX_AMT, true)+" 원");
						$(".card:eq(1) .right").html(avatar.common.comma(rec.MAX_AMT, true)+" 원");
						$(".aias_news td span:eq(1)").html(avatar.common.comma(rec.MAX_AMT, true)+" 원");
						$(".aias_news td span:eq(0)").html(rec.BUYR_CORP_NM);
						if(avatar.common.null2void(date1)!="")
							$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1) +" 최고매출 거래처는");
						else{
							$(".aias_news strong").html(date_format_kr(date) +" 최고매출 거래처는");
