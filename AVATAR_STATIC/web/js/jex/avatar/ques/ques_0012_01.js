...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"] = date;
			input["END_DT"] = date1;
			avatar.common.callJexAjax("ques_0012_01_r001", input, _thisPage.fn_callback, "true", "");
			...
			if(dat.REC2[1].CNT==0){
				...
				if(dat.REC.length == 0){
					if(avatar.common.null2void(date1)!=""){
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1) +"카드매출은");
					}else{
						$(".aias_news strong").html(date_format_kr(date)+"카드매출은");
					}
					...
					$.each(dat.REC, function(idx, rec){
						html= '';
						html+='<dd>';
						html+=	rec.CARD_CORP_NM + "("+rec.AMT_CNT+"건)";
						html+='	<div class="right">';
						html+=	avatar.common.comma(rec.AMT_SUM,true)+'원';
						html+='	</div>';
						html+='</dd>';
