...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"] = date;
			input["END_DT"] = date1;
			avatar.common.callJexAjax("ques_0004_01_r001", input, _thisPage.fn_callback, "false", "c");
		},
		...
		fn_callback : function(data){
			if((data.REC2[0].CNT == 0) && (data.REC2[1].CNT == 0)){
				$(".data_y").hide();
				$(".data_n").show();
			}
			else{
				...
				$(".aias_list dd:eq(1) .right").html(avatar.common.comma(cashAmt, true)+" 원");
				$(".aias_list dd:eq(2) .right").html(avatar.common.comma(cardAmt, true)+" 원");
				$(".c_blue").html(avatar.common.comma(totlAmt, true)+"원");
				
				if(avatar.common.null2void(date1)!="")
					$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1)+ "매출액은");
				else
					$(".aias_news strong").html(date_format_kr(date)+ "매출액은");
