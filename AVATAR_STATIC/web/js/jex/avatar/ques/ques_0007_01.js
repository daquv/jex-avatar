 ...
 *
 * @File Name      : ques_0007_01.js
 * @File path      : AVATAR_STATIC/web/js/jex/avatar/ques
 * @author         : 김별 (  )
 * @Description    : 카드결제금액화면/조회
 * @History        : 20200122132020, 김별
 * </pre>
 **/
var date = "";
var date1 = "";
...
var _thisPage = {
		...
		searchData : function(){
			...
			input["STR_DT"] = date;
			input["END_DT"] = date1;
			avatar.common.callJexAjax("ques_0007_01_r001", input, _thisPage.fn_callback, "false", "c");
		},
			...
			if(data.REC2.length == 0){
				...
				if(data.REC.length == 0){
					if(avatar.common.null2void(date1)!="")
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1) + "카드결제 금액은");
					else{
						$(".aias_news strong").html(date_format_kr(date)+ "카드결제 금액은");
					}
					$(".aias_news td .c_red").html("0원");
				} else{
					var totlAmt = 0;
					$.each(data.REC, function(idx, rec){
						var html = '';
						html += '<dd>';
						html += 	rec.BANK_CD + "("+rec.BUY_CNT+"건)";
						html += '	<div class="right">';
						html += 		avatar.common.comma(rec.BUY_SUM, true)+"원";
						html += '	</div>';
						html += '</dd>';
						
						$(".aias_list dl").append(html);
						
						totlAmt += parseInt(rec.BUY_SUM);
					});
					$(".aias_news td .c_red").html(avatar.common.comma(totlAmt, true)+"원");
					if(avatar.common.null2void(date1)!="")
						$(".aias_news strong").html(date_format_kr(date)+" ~ " + date_format_kr(date1) + "카드결제 금액은");
					else{
						$(".aias_news strong").html(date_format_kr(date)+ "카드결제 금액은");
