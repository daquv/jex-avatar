...
var index = -1;
$(function(){
	iWebAction("changeTitle",{"_title" : "카드매출", "_type" : "2"});
	_thisPage.onload();
...
var _thisPage = {
		...
		searchData : function(){
			...
			avatar.common.callJexAjax("card_0001_01_r001", input, _thisPage.fn_callback, "true", "");
				...
				$(".data_n").show();
					...
					$.each(dat.REC, function(idx, rec){
						...
						if(dat.REC[idx].TRNS_DT == trnsDt[idx] && lastdate != dat.REC[idx].TRNS_DT){
							...
							tempHtml += '						<td><span class="elipsis">'+rec.CARD_CORP_NM+'('+rec.CARD_NO.substring(rec.CARD_NO.length, rec.CARD_NO.length-4)+')</span></td>';
							tempHtml += '						<td class="won"><em>'+avatar.common.comma(rec.APV_AMT,true)+'</em>원</td>';
							tempHtml += '					</tr>';
							tempHtml += '			</table>';
							tempHtml += '		</div>';
							tempHtml += '	</td>';
							...
							tempHtml2 += '					<tr>';
							tempHtml2 += '						<th></th>';
							tempHtml2 += '						<td><span class="elipsis">'+rec.CARD_CORP_NM+'('+rec.CARD_NO.substring(rec.CARD_NO.length, rec.CARD_NO.length-4)+')</span></td>';
							tempHtml2 += '						<td class="won"><em>'+avatar.common.comma(rec.APV_AMT,true)+'</em>원</td>';
							tempHtml2 += '					<tr>';
							/*index = -1;
							if(trnsDt[idx] != null){
								index++;
