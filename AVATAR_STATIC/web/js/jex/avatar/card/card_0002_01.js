 ...
 *
 * @File Name      : card_0002_01.js
 * @File path      : AVATAR_STATIC/web/js/jex/avatar/card
 * @author         : 김별 (  )
 * @Description    : 데이터_카드매입목록화면
 * @History        : 20200120164407, 김별
 * </pre>
 **/
...
var index = -1;
$(function(){
	iWebAction("changeTitle",{"_title" : "카드매입", "_type" : "2"});
	_thisPage.onload();
...
var _thisPage = {
		...
		searchData : function(){
			...
			avatar.common.callJexAjax("card_0002_01_r001", input, _thisPage.fn_callback, "", "");
		...
		fn_callback : function(dat){
			...
			if(dat.TOTL_CNT == 0){
				...
				if(dat.TOTL_CNT <= (pageno-1)*pagecnt){
					return false;
				}
				else{
					/* 날짜별 */
					var issuDt = [];
					var k = 0;
					issuDt[0] = dat.REC[0].APV_DT;
					for(i=0; i<dat.REC.length-1 ; i++){
						if(dat.REC[i].APV_DT != dat.REC[i+1].APV_DT){
							issuDt[i+1] = dat.REC[i+1].APV_DT;
						} else{
							issuDt[i+1] = '';
						}
					}
					...
					$.each(dat.REC, function(idx, rec){
						...
						if(dat.REC[idx].APV_DT == issuDt[idx] && lastdate != dat.REC[idx].APV_DT){
							console.log('start :: '+issuDt[idx] );
							...
							tempHtml += '					<tr>';
							tempHtml += '						<th>'+ avatar.common.date_format2(rec.APV_DT).substring(5) +'</th>';
							tempHtml += '						<td><span class="elipsis">'+rec.MEST_NM+'</span></td>';
							tempHtml += '						<td class="won"><em>'+avatar.common.comma(rec.BUY_SUM,true)+'</em>원</td>';
							tempHtml += '					</tr>';
							tempHtml += '			</table>';
							tempHtml += '		</div>';
							tempHtml += '	</td>';
						...
						else if(dat.REC[idx].APV_DT != issuDt[idx] || lastdate == dat.REC[idx].APV_DT){
							console.log('next :: '+issuDt[idx] + " " +dat.REC[idx].APV_DT);
							tempHtml2 += '					<tr>';
							tempHtml2 += '						<th></th>';
							tempHtml2 += '						<td><span class="elipsis">'+rec.MEST_NM+'</span></td>';
							tempHtml2 += '						<td class="won"><em>'+avatar.common.comma(rec.BUY_SUM,true)+'</em>원</td>';
							tempHtml2 += '					<tr>';
							index = -1;
							/*if(issuDt[idx] != null){
								index++;
