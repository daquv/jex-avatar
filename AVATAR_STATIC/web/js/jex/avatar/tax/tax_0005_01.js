...
var index = -1;
$(function(){
	iWebAction("changeTitle",{"_title" : "매출현금영수증", "_type" : "2"});
	_thisPage.onload();
...
var _thisPage = {
		...
		searchData : function(){
			...
			avatar.common.callJexAjax("tax_0005_01_r001", input, _thisPage.fn_callback, "true", "");
			...
			if(dat.TOTL_CNT == 0){
				...
				$.each(dat.REC, function(idx, rec){
					...
					if(dat.REC[idx].TRNS_DT == trnsDt[idx] && lastdate != dat.REC[idx].TRNS_DT){
						...
						tempHtml += '						<td><span class="elipsis">'+rec.MEMO+'</span></td>';
					...
					else if(dat.REC[idx].TRNS_DT != trnsDt[idx]){
						tempHtml2 += '					<tr>';
						tempHtml2 += '						<th></th>';
						tempHtml2 += '						<td><span class="elipsis">'+rec.MEMO+'</span></td>';
