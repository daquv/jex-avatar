...
//        datePicker.toDate = new Date(datePicker.today.getFullYear(), datePicker.today.getMonth(), datePicker.getLastDate(datePicker.today.getYear(), datePicker.today.getMonth() + 1));
        ...
        var dpDefaultSetting = {
            ...
            changeMonth: true,
            showOn: "button",
            buttonImage: "/admin/img/ico/ico_calendar.png",
...
//		});
	...
	,
	setCalendar : function(selector, date, type){
		if(type == null || type == undefined) type = "yy-mm-dd";
		if(type == "yy-mm-dd") {
			$(selector).datepicker({
				showOn: "button",
	            buttonImage: "/admin/img/ico/ico_calendar.png",
			...
			datepicker_default.beforeShow = function () {
				var selectDate = $(selector).val().split("-");
				var year = Number(selectDate[0]);
				var month = Number(selectDate[1]) - 1;
				$(selector).datepicker( "option", "defaultDate", new Date(year, month, 1) );
				$(selector).val(selectDate[0] + "-" + selectDate[1]);
			}
			$(selector).datepicker(datepicker_default);
...
$(function() {
	$(document).on("click", ".ui-datepicker-close", function(e) {
		var month = $("#ui-datepicker-div .ui-datepicker-month :selected").val();
		var year = $("#ui-datepicker-div .ui-datepicker-year :selected").val();
		__calendar_thisObj.datepicker( "option", "defaultDate", new Date(year, month, 1) );
		__calendar_thisObj.datepicker('setDate', new Date(year, month, 1));
	});
})