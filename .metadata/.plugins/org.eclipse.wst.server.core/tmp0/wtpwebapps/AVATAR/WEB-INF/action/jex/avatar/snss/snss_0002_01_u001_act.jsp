...
<%
         ...
         *
         * @File Title   : 온라인매출 변경
         * @File Name    : snss_0002_01_u001_act.jsp
         * @File path    : snss
         * @author       : jepark (  )
         * @Description  : 온라인매출 변경
         * @Register Date: 20210722150250
         * </pre>
         *
         * ============================================================
         * 참조
        ...
        @JexDataInfo(id="snss_0002_01_u001", type=JexDataType.WSVC)
    	...
    	try{
	    	...
	    	if("00000000".equals(rsltCd) || "42110000".equals(rsltCd)){
		    	...
		    	if("00000000".equals(rsltCd) || "42110000".equals(rsltCd)){
		    		if("1".equals(stts)){
		    			//온라인매출 계정 수정
		    			resData = CooconApi.updateSmart(bsnn_no, shop_cd, sub_shop_cd, web_id, web_pwd);
	    			}else{
	    				//온라인매출 계정 등록
	    			...
	    			if("00000000".equals(resData.getString("ERRCODE"))
						...
						// 온라인매출계정 수정
				        JexData idoIn1 = util.createIDOData("EVDC_INFM_U016");
	    		        ...
	    		        if (DomainUtil.isError(idoOut1)) {
							...
							result.put("RSLT_MSG", "온라인 매출 증빙정보 등록 시 오류가 발생했습니다.");
	    		        }else{
	    		        	// 아바타 서버에 계정정보 등록 성공
